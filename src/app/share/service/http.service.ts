import { LOCAL_STORAGE, AESINFO, HTTPResponseCode } from './../constant/constant';
    ...
    return new Promise((resolve, reject) => {
      // if(this.authService.hasSession()) {
        ...
        this.data = this.httpClient.post(uri, dataBody, {
          ...
          $('body').addClass('loaded');
          $('div.loading').addClass('none');
          const httpCode = error.error.status;
          if(httpCode === 404) {
            alert('API Not Found['+error.error.path+']');
          } else {
            alert(error.error.error+'['+error.error.error+']');
          }
          console.log(error);
        });
      // } else {
